...
const ViewProfileScreen = ({ navigation, route }) => {
  ...
  return (
    <StatusBarWrapper>
      ...
      >
        <CustomHeader
          title={item?.name || "Teacher Profile"}
          rightIcon={<MoreVertical size={22} color="#000" />}
        />
        <ScrollView
        ...
        <TouchableOpacity style={styles.btn}>
          <Text style={styles.btnText}>
            {isTeacher
              ? t(`teacherProfile.acceptRequest`)
              : t(`teacherProfile.sendRequest`)}
          </Text>
        </TouchableOpacity>
      </View>
    </StatusBarWrapper>
